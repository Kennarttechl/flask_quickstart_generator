...
# ANSI escape color code for displaying or printing successful message
GREEN = "\033[92m"
RESET = "\033[0m"
...
def main():
    ...
    command = args[0].strip() if len(args) > 0 else ""
    argument = args[1].strip() if len(args) > 1 else ""
    project_name = args[2].strip() if len(args) > 2 else ""

    if command == "-v" and argument == "create-app" and project_name:
        CmdHandler.init()
        CmdHandler.generate_flask_app_folder(project_name)
        print(f"{GREEN}***** Virtual environment and project created successfully *****{RESET}")
        ...
        CmdHandler.flask_db_init()
        print(f"{GREEN}***** Database migration created successfully *****{RESET}")
